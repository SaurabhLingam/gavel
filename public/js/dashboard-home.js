...
(async function() {
    ...
    const endingSoon = await api.get('/auctions?status=active&sort=endTime&order=asc&limit=6');
...
function renderActivityStrip(bids, watchlist) {
    ...
    bids.forEach(bid => {
        if (bid.auctionStatus === 'active') {
            ...
            // Placeholder
            winning.push(bid);
        } else if (bid.auctionStatus === 'closed' && bid.winnerEmail !== window.__dashboardUser?.email) {
            outbid.push(bid);
        }
    });
...
function renderEndingSoon(auctions) {
    ...
    if (!auctions || !auctions.length) {
        container.innerHTML = '<div class="empty-state"><h3>No live auctions ending soon</h3><p>Fresh listings will appear here automatically.</p></div>';
        return;
    }
