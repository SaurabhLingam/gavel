    ...
    wss.on('connection', (ws, req) => {
        ...
        if (token && JWT_SECRET) {
            try {
                ...
                registerPresence(ws.userEmail, ws);
            } catch (e) { /* anonymous */ }
        }
            ...
            try {
                ...
                switch (msg.type) {
                    ...
                    case 'register_presence':
                        if (msg.email) {
                            ws.userEmail = msg.email;
                            registerPresence(msg.email, ws);
                        }
                        break;
                }
            } catch (e) {
        ...
        ws.on('close', () => {
            ...
            unregisterPresence(ws.userEmail, ws);
...
function registerPresence(email, ws) {
    if (!email) return;
    if (!presenceByEmail.has(email)) presenceByEmail.set(email, new Set());
    presenceByEmail.get(email).add(ws);
}

function unregisterPresence(email, ws) {
    if (!email || !presenceByEmail.has(email)) return;
    const sockets = presenceByEmail.get(email);
    sockets.delete(ws);
    if (sockets.size === 0) presenceByEmail.delete(email);
}

function isUserOnline(email) {
    return Boolean(email && presenceByEmail.has(email) && presenceByEmail.get(email).size);
}

function getOnlineUserEmails() {
    return Array.from(presenceByEmail.keys());
}
...
module.exports = {
    ...
    isUserOnline, getOnlineUserEmails
};
