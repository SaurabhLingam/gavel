    ...
    try {
        if (JWT_SECRET) {
            try {
                ...
                if (dbUser) return res.json({ loggedIn: true, user: { id: dbUser._id, email: dbUser.email, name: dbUser.fullname, role: dbUser.role, isAdmin: dbUser.isAdmin || dbUser.isSuperAdmin, isSuperAdmin: dbUser.isSuperAdmin, phoneNumber: dbUser.phoneNumber || '', phoneVerified: Boolean(dbUser.phoneVerification?.verified) } });
            } catch (e) { /* fallthrough */ }
        }
        if (supabase) {
            ...
            if (data?.user) {
                ...
                if (dbUser) return res.json({ loggedIn: true, user: { id: dbUser._id, email: dbUser.email, name: dbUser.fullname, role: dbUser.role, isAdmin: dbUser.isAdmin || dbUser.isSuperAdmin, isSuperAdmin: dbUser.isSuperAdmin, phoneNumber: dbUser.phoneNumber || '', phoneVerified: Boolean(dbUser.phoneVerification?.verified) } });
                return res.json({ loggedIn: true, user: { email: data.user.email, name: data.user.user_metadata?.full_name || data.user.email, role: 'bidder', phoneNumber: '', phoneVerified: false } });
            }
        }
    ...
    try {
        ...
        const existing = await User.findOne({ email });
         ...
         if (existing) {
            ...
            return res.json({ success: true, user: { id: existing._id, email: existing.email, name: existing.fullname, isAdmin: existing.isAdmin || existing.isSuperAdmin, isSuperAdmin: existing.isSuperAdmin, phoneNumber: existing.phoneNumber || '', phoneVerified: Boolean(existing.phoneVerification?.verified) } });
        }
        ...
        res.cookie('sb_access_token', token, cookieOptsShort);
        res.json({ success: true, user: { id: newUser._id, email: newUser.email, name: newUser.fullname, isAdmin: newUser.isAdmin || newUser.isSuperAdmin, isSuperAdmin: newUser.isSuperAdmin, phoneNumber: newUser.phoneNumber || '', phoneVerified: Boolean(newUser.phoneVerification?.verified) } });
