const UI = {
    ...
    renderAuctionCard(auction) {
        ...
        const specs = this.normalizeSpecifications(auction.specifications);
        const specsMarkup = specs.length ? `
            <dl class="auction-card-specs" style="display:grid;grid-template-columns:auto 1fr;gap:0.25rem 0.5rem;margin-top:0.75rem;font-size:var(--text-xs);">
                ${specs.map(([key, value]) => `<dt style="color:var(--text-muted);font-weight:600;">${key}</dt><dd style="margin:0;color:var(--text-secondary);">${value}</dd>`).join('')}
            </dl>` : '';
        ...
        <a href="item-detail.html?id=${auction.id}" class="card" style="text-decoration:none;color:inherit;">
          ...
          <div class="card-body">
            ...
            ${auction.description ? `<p class="text-muted" style="font-size:var(--text-xs);margin-top:0.5rem;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;">${auction.description}</p>` : ''}
            ${specsMarkup}
    ...
    normalizeSpecifications(specifications) {
        ...
        if (Array.isArray(specifications)) {
            return specifications
                .map((value, index) => [`Spec ${index + 1}`, String(value || '').trim()])
                .filter(([, value]) => value);
        }
