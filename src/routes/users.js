...
router.get('/profile', requireLogin, async (req, res) => {
    try {
        ...
        const completedTransactions = await Auction.countDocuments({ status: 'closed', $or: [{ winnerEmail: user.email }, { sellerEmail: user.email }] });
        const campusUsersAhead = await User.countDocuments({ college: user.college, trustScore: { $gt: Number(user.trustScore || 0) } });
        const monthlyWon = await Auction.find({
            winnerEmail: user.email,
            status: 'closed',
            updatedAt: { $gte: new Date(Date.now() - 180 * 24 * 60 * 60 * 1000) }
        }).select('winningBid updatedAt');
        const monthlySold = await Auction.find({
        ...
        const completenessParts = [
            ...
            Boolean(user.bio),
            Boolean(user.location && (user.location.city || user.location.country)),
            Boolean(user.avatar)
        ];
        const profileCompleteness = Math.round((completenessParts.filter(Boolean).length / completenessParts.length) * 100);
        ...
        res.json({
            ...
            stats: { activeListings, closedListings, pendingListings, rejectedListings, totalBids, activeBids: activeBids.length, auctionsWon, watchlistCount, completedTransactions },
            gamification: {
                biddingStreak: Math.min(30, totalBids),
                campusRank: campusUsersAhead + 1,
                profileCompleteness,
                monthlyWon,
                monthlySold
            },
